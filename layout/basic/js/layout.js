window.addEventListener('load', function(){
    ...
    /* 등급에 따라 카테고리 메뉴 동적(임직원,관리자인 경우만) 출력 */
    try {
        // 출력 대상 등급 리스트
        var user_chk_grp_list = ["임직원", "관리자"];

        // 사용자 등급명
        var user_grp_name = CAPP_ASYNC_METHODS.member.getData().group_name;
        console.log(CAPP_ASYNC_METHODS.member.getData());
        console.log("user_grp_name: ", user_grp_name);
        // 로그인 상태이면서 등급명이 대상 등급에 포함될 경우
        if (CAPP_ASYNC_METHODS.IS_LOGIN === true && user_chk_grp_list.includes(user_grp_name)) {
            $("li[data-cate='257']").show();
            $("li[data-cate='259']").show();
        }
    } catch(e) {
        console.log("cate menu error", e);
    }
});
...
function bottomNav(){
    ...
    window.addEventListener("scroll", function(){
        ...
        // scroll bottom
        /*8if(scroll === document.body.scrollHeight - document.documentElement.offsetHeight){
			nav.classList.remove('bottom-nav--hide');
		}*/
        if(scroll >= document.body.scrollHeight - document.documentElement.offsetHeight - $('footer').outerHeight()){
            nav.classList.add('bottom-nav--hide');
        }
...
jQuery(document).ready(function() {
    ...
    jQuery(".mypage-board-list").each(function () {
        var itemCount = jQuery("ul > li", this).length;

        if (itemCount === 0) {
            jQuery(this).find(".empty-text").css("display", "flex");
        } else {
            jQuery(this).find(".empty-text").hide();
        }
    });
